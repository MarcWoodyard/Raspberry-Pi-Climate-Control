...
public class ConfigImporter {
	...
	private void importServoConfig() {
		try {
			servoScanner = new Scanner(new File("./data/config/ServoConfig.txt"));
	...
	private void importDHT11Config() {
		try {
			dht11Scanner = new Scanner(new File("./data/config/DHT11Config.txt"));
	...
	private void importRaspstillConfig() {
		try {
			raspstillScanner = new Scanner(new File("./data/config/RaspstillConfig.txt"));
	...
	private void importProgramConfig() {
		try {
			programScanner = new Scanner(new File("./data/config/ProgramConfig.txt"));
	...
	/**
	 * Returns the number of miliseconds to wait while the servo is turning the AC
	 * on.
	 * 
	...
	/**
	 * Returns the amount of minutes the program should wait before checking the
	 * room temperature.
	 * 
	 * @return int - Amount of minutes program will wait before checking the
	 *         temperature.
	 */
	...
	/**
	 * Returns the max temperature the room can get that will turn on the air
	 * conditioner.
	 * 
	...
	/**
	 * Returns the min temperature the room can get that will turn off the air
	 * conditioner.
	 * 
