...
public class ImageAnalyzer {
    ...
    /**
     * Creates a TreeMap<Color (Key), Times in Picture (Value)> and maps out the
     * pixels in a image.
     *
     * @param File - A file object linking to a picture.
     * @return TreeMap<Integer, Integer> - TreeMap containing pixel color values
     * with the amount contained in the picture.
     */
    public TreeMap<Integer, Integer> mapPixels(File picture) {
        TreeMap<Integer, Integer> picColors = new TreeMap<>();
        ...
        try {
            ...
            for (int h = 0; h < image.getTileHeight(); h++) {
                for (int w = 0; w < image.getTileWidth(); w++) {
                    ...
                    if (picColors.containsKey(image.getRGB(w, h))) {
     ...
     * @return ArrayList<String> - ArrayList containing the color names converted
     * from a decimal color code.
     */
